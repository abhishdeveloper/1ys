    ...
    <main class="flex-grow">
        ...
        <?php if (empty($products)): ?>
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-12 text-center">
                ...
                <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white"><?php echo __('no_products'); ?></h3>
                <div class="mt-6">
        ...
        <?php else: ?>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-10 gap-x-6">
                <?php foreach ($products as $product): ?>
                    <div class="group relative bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden flex flex-col transition-all duration-300 hover:shadow-xl hover:-translate-y-2">
                        <!-- Image -->
                        <div class="w-full min-h-[16rem] bg-gray-50 dark:bg-gray-700 aspect-w-1 aspect-h-1 overflow-hidden relative">
                            <?php if ($product['image_url']): ?>
                                <img src="<?php echo sanitize($product['image_url']); ?>" alt="<?php echo sanitize($product['name']); ?>" class="w-full h-full object-center object-contain p-4 transition-transform duration-500 group-hover:scale-110">
                            <?php else: ?>
                            ...
                            <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-10 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                                <a href="/product/<?php echo sanitize($product['slug']); ?>" class="bg-white text-gray-900 font-semibold py-2 px-4 rounded-full shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 z-20 hover:bg-green-50 hover:text-primary">
                                    <?php echo __('view_all_products'); ?>
                                </a>
                            </div>
                        </div>
                        <!-- Details -->
                        <div class="p-6 flex-grow flex flex-col justify-between bg-white dark:bg-gray-800 z-10 relative">
                            <div>
                                <p class="text-xs text-primary dark:text-green-400 font-semibold uppercase tracking-wider mb-2"><?php echo sanitize($product['category_name'] ?? 'Uncategorized'); ?></p>
                            ...
                            <div class="mt-4 flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
                                <p class="text-xl font-extrabold text-gray-900 dark:text-white">₹<?php echo number_format($product['price'], 2); ?></p>
                                <form action="/cart/add" method="POST" class="relative z-20">
                                    <input type="hidden" name="product_id" value="<?php echo $product['id']; ?>">
                                    <input type="hidden" name="quantity" value="1">
                                    <button type="submit" class="p-3 rounded-full bg-green-50 text-primary hover:bg-primary hover:text-white transition-all transform hover:scale-110 focus:outline-none shadow-sm" aria-label="Add to cart">
